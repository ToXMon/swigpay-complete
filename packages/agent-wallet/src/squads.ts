...
export async function provisionAgentWallet(params: {
  ...
  // Agent: Initiate + Execute (can act autonomously within spending limit)
  // Human: All permissions (full override authority)
  const sig = await multisig.rpc.multisigCreateV2({
    connection,
    treasury: configTreasury,
    createKey,
    creator: humanKeypair,          // pays rent, configAuthority
    multisigPda,
    configAuthority: humanKeypair.publicKey,
    threshold: 1,                    // 1-of-2 threshold
    members: [
  ...
  // ---- 3. Add USDC spending limit for the agent ----
  ...
  const dailyLimitRaw = BigInt(Math.round(dailyLimitUsdc * USDC_RAW_MULTIPLIER)); // USDC has 6 decimals
  ...
  const config: AgentConfig = {
    ...
    dailyLimitUsdc,
    perTxLimitUsdc: dailyLimitUsdc / 10,   // per-tx = 10% of daily
    approvalThresholdUsdc: dailyLimitUsdc * 0.5, // approval above 50% of daily
  ...
  console.log(`   Send USDC to: ${config.vaultPda} (Solana devnet)`);
